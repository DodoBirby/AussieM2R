...
if (kFire && kFirePushedSteps == 1 && (state == 23 || state == 24 || state == 27 || state == GRABBEDQUEENMORPH || state == GRABBEDQUEENBELLY) && (!instance_exists(oEMPNoise)))
{
    ...
    if (((global.opmslstyle == 0 && armmsl == 1) || (global.opmslstyle == 1 && global.currentweapon == 3)) && global.pbombs > 0 && instance_number(oQueenFinalExplosion) == 0 && state != GRABBEDQUEENMORPH && (!((state == GRABBEDQUEENBELLY && distance_to_point((oQueenFront.x + 81), y) > 32))))
    {
        if global.saxmode
        {
...
var modifier = 0
var actualDir = aimdir_to_dir(aimdirection)
...
if (instance_number(oChargeBeamSpark1) == 0 && chargebeam > 0)
    instance_create((x + aspr2x), (y - aspr2y - 39 + modifier), oChargeBeamSpark1)
damage = 15
if global.wbeam
    damage *= 1.5
if global.ibeam
