if (state == SPIDERBALL)
{
    ...
    if (isCollisionRight(0) == 1)
        x -= 1
    if ((isCollisionBottom(1) == 1 || isCollisionPlatformBottom(1) == 1) && isCollisionRight(1) == 0 && isCollisionLeft(1) == 0)
        sbstate = 1
    ...
    if (kRight == 0 && kLeft == 0 && kUp == 0 && kDown == 0)
        sbmove = 0
    if (sbstate == 1)
        x += sbmove
    if (sbstate == 2)
...
{
    if (isCollisionTop(1) == 1 || isCollisionBottom(1) == 1 || isCollisionRight(1) == 1 || isCollisionLeft(1) == 1)
        state = SPIDERBALL
}
