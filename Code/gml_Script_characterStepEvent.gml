...
if (isCollisionTop(1) || isCollisionPlatformTop(1))
    collision_bottom = 1
else
    collision_bottom = 0
SetSpinJumpSound()
...
if ((isCollisionTop(1) || isCollisionPlatformTop(1)) && platformCharacterIs(IN_AIR) && yVel >= 0)
{
...
if (isCollisionTop(1) == 0 && isCollisionPlatformTop(1) == 0 && platformCharacterIs(ON_GROUND))
{
    xAcc = xVel / 1.2
    xVel *= 0.5
...
if (state == BALL || state == AIRBALL)
{
    ...
    if (platformCharacterIs(ON_GROUND) && kJump && kJumpPushedSteps == 0 && global.jumpball == 1 && state == BALL)
    {
        ...
        if (monster_drain > 0)
            jump_vel = -1
        if (isCollisionBottom(abs(jump_vel) + 1) == 0)
            yVel = jump_vel
        else if (isCollisionBottom(abs(jump_vel)) == 0)
            yVel = jump_vel + 3
        else if (isCollisionBottom(abs(jump_vel) - 1) == 0)
            yVel = jump_vel + 4
        else if (isCollisionBottom(abs(jump_vel) - 2) == 0)
            yVel = jump_vel + 5
        else if (isCollisionBottom(abs(jump_vel) - 3) == 0)
            yVel = jump_vel + 6
        state = AIRBALL
        statetime = 0
        sfx_play(sndBallBounce)
...
if (state == SUPERJUMP)
{
    ...
    else if ((isCollisionLeft(1) && (sjdir == 2 || sjdir == 4)) || (isCollisionRight(1) && (sjdir == 1 || sjdir == 3)) || (isCollisionBottom(1) && (sjdir == 0 || sjdir == 3 || sjdir == 4)))
    {
        state = SJEND
        statetime = 0
    }
...
if (state == WATERJET)
{
    ...
    if (statetime > 120)
        yVel = -8
    if (isCollisionBottom(1) > 0 && statetime > 140)
        xVel = 8
    else
        xVel = 0
    if (statetime == 500)
...
{
    dash = 0
    if (isCollisionTop(1) == 0 && (isCollisionPlatformTop(1) == 0 || isCollisionPlatform()))
    {
        xAcc = xVel / 1.2
        xVel *= 0.5
        canturn = 1
...
{
    if ((cmhurt > 0 || cmmorph > 0) && kJump && kJumpPushedSteps == 0 && isCollisionTop(1) == 0)
    {
        if (sjball == 0)
        {
            state = JUMPING
...
if ((maxSlope * abs(xVel)) > 0 && (platformCharacterIs(ON_GROUND) || state == BRAKING))
{
    ...
    moveTo(xVel, -yVel)
if (isCollisionTop(1) == 0 && (maxDownSlope * abs(xVelInteger)) > 0 && (platformCharacterIs(ON_GROUND) || state == BRAKING))
{
    upYPrev = y
