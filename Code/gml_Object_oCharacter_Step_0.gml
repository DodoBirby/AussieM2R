...
if instance_exists(oBlockStep)
{
    if platformCharacterIs(ON_GROUND)
    {
        if (dash == 0)
        {
            block_id = whatIsCollidingTop(1)
            if (block_id != noone && block_id.object_index != oBlockStep) {
                block_id = noone
            }
        }
            
        
        // just removing this else because I literally do not care about crumbles when speedboosting
        // else
        // {
